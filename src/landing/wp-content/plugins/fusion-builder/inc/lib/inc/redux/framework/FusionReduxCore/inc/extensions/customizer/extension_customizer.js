...
(function( $ ) {  //This functions first parameter is named $
    ...
    fusionredux.customizer = fusionredux.customizer || {};

    $( document ).ready(
        function() {
            fusionredux.customizer.init();
        }
    );
    fusionredux.customizer.init = function() {
        $( 'body' ).addClass( fusionredux_customizer.body_class );
        $( '.accordion-section.fusionredux-section, .accordion-section.fusionredux-panel, .accordion-section-title' ).click(
            function() {
                $.fusionredux.initFields();
            }
        );
    };
    fusionredux.customizer.save = function( $obj, $container ) {
        var $parent = $obj.hasClass( 'fusionredux-field' ) ? $obj : $obj.parents( '.fusionredux-field-container:first' );
        var $id = $parent.parent().find( '.fusionredux-customizer-input' ).data( 'id' );
