...
export class SiteContactsComponent implements OnInit, OnChanges {
    ...
    protected adfEmailEntities: ContactEntity[];
    protected newAdfEmailEntity: ContactEntity;
    ...
    ngOnChanges(changes: SimpleChanges) {
        ...
        if (Variable.isNotNullOrUndefined(siteContactsChange) && this._contacts !== this.siteContacts) {
    ...
    protected initializeNotificationEntities() {
        ...
        this.initializeAdfEmailEntities();
    }

    protected saveSiteContacts(): void {
        const value = `${this.emailEntities.map((item) => item.value).join(',')};${this.smsEntities.map((item) => item.value).join(',')};${this.adfEmailEntities.map((item) => item.value).join(',')};`;

        this.onSaveSiteContacts.emit(value);
    }
    ...
    // adf email contacts
    protected initializeAdfEmailEntities(): void {
        this.adfEmailEntities = this.parseContactEntitiesString(this._contacts, ';', ',', 2);
        this.newAdfEmailEntity = new ContactEntity('');
    }
    ...
    protected addNewAdfEmailEntity(): void {
        this.adfEmailEntities.push(new ContactEntity(this.newAdfEmailEntity.value));
        this.newAdfEmailEntity.value = '';
    }

    /// predicates
    protected isAdfEmailValid(value: string): boolean {
        return Extensions.regExp.email.test(value) && !this.containsContactsEntityValue(this.adfEmailEntities, value);
    }
